...
import { GMArea, AGMButton, GraphFeature, GMTitle, GMSelect, GMTop, GMRH, GMCircle, GraphFeatureHold, StockPrice } from "./styled"
...
const ContainerGraphView = ({ getcmp, dyntick, peers, brewort, central, eraseContainer, containers, theme, dyntrend, activeTheme }) => {
        ...
        {(provided, snapshot) => (
          <GMCircle ref={provided.innerRef} {...provided.draggableProps}>
            <GMArea bcolor={theme.graphColor} size={{ width: bsw, height: bsh }} maxHeight={bsmh} maxWidth={bsmw} minHeight={bslh} minWidth={bslw} enable={{ top: false, right: false, bottom: false, left: false, topRight: false, bottomRight: true, bottomLeft: false, topLeft: false }}
              ...
              <GMTop color="#0fc4ac"><GraphFeature icon={faSave} style={{visibility: 'hidden'}} fcolor={theme.fontColor}></GraphFeature>
                <div {...provided.dragHandleProps}><GraphFeatureHold icon={faGripLines} fcolor={theme.fontColor}></GraphFeatureHold></div>
                <GraphFeature icon={faTrashAlt} fcolor={theme.fontColor} onClick={() => containerOnDel(dsid)}></GraphFeature></GMTop>
                {central.map((grafData, j) => {
                  if(grafData.status === 'ok' && grafData.dsid === dsid && grafData.gtype === 'candle') {
                    ...
                    return <React.Fragment key={j}>
                      ...
                      <div style={{display: 'flex', justifyContent: 'center'}}><GMSelect onChange={(e) => updateData(grafData.dsid, grafData.compname, e)} options={peers.find(p => p.compname === grafData.compname).periods}></GMSelect></div>
                  ...
                  return <React.Fragment key={j}>
                    <StockPrice isDark={activeTheme}>Price: ${grafData.latestPrice}</StockPrice>
                    <GMTitle style={{fontSize: '2.5rem', opacity: '0.7'}} color={theme.fontColor}>{grafData.compname.label}</GMTitle>
...
const mapStateToProps = (state) => {
  return {
    ...
    activeTheme: state.darkModeToggler.toggle,
